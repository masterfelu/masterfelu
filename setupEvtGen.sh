...
# Base location for build
BUILD_BASE="$HOME/evtgen_buildbase"

# Location in which to install
INSTALL_PREFIX="$HOME/.local"

# EvtGen local directory, folder name should be evtgen
EVTGEN_LOCAL="$HOME/evtgen"
...
echo Will install EvtGen and its dependencies in $INSTALL_PREFIX

echo Build area is $BUILD_BASE
...
echo Copying EvtGen source from local directory $EVTGEN_LOCAL
cd sources
cp -r $EVTGEN_LOCAL .
cd evtgen
